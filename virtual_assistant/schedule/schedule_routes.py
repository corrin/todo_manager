    ...
    def generate_and_add_to_calendar():
        ...
        try:
            ...
            calendar_account = ExternalAccount.query.filter_by(
                user_id=current_user.id, use_for_calendar=True, is_primary_calendar=True
            ).first()

            if not calendar_account:
                return (
                    jsonify({"error": "No primary calendar account found. Please set up a calendar in Settings."}),
                    400,
                )
            ...
            if calendar_account.provider == "google":
                ...
                return (
                    jsonify({"error": f"Unsupported calendar provider: {calendar_account.provider}"}),
                    400,
                )
            ...
            return jsonify({"schedule": schedule, "added_events": added_events})

        except Exception as e:
            logger.error(f"Error generating schedule and adding to calendar: {e}")
            flash(
                "An error occurred while generating schedule and adding to calendar.",
                "danger",
            )
            return (
                jsonify({"error": "Failed to generate schedule and add to calendar"}),
                500,
            )
