...
def create_app():
    ...
    token_refresh_thread = threading.Thread(target=run_token_refresh_scheduler, daemon=True)
...
def settings():
    ...
    return render_template("settings.html", user=current_user, external_accounts=external_accounts)
...
def login():
    ...
    try:
        ...
        return (
            jsonify({"success": False, "error": "An internal error occurred during login."}),
            500,
        )
