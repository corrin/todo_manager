...
class OutlookTaskProvider(TaskProvider):
    ...
    def authenticate(self, user_id, task_user_email):
        ...
        if account.needs_reauth:
            logger.info(
                f"O365 account for user_id '{user_id}' / task_user_email '{task_user_email}' needs reauthorization"
            )
            ...
            return self.provider_name, redirect(
                url_for(
                    ...
                    provider="o365",
                    calendar_email=account.external_email,
                )
            )
    ...
    def get_tasks(self, user_id, task_user_email) -> List[Task]:
        ...
        try:
            ...
            for t in all_tasks:
                ...
                priority = {"low": 1, "normal": 2, "high": 3, "urgent": 4}.get(t.get("importance", "normal"), 2)
            ...
            logger.error(
                f"Error getting Outlook tasks for user_id '{user_id}' / task_user_email '{task_user_email}': {e}"
            )
            raise
        ...
        try:
            ...
            if "due_date" in task_data:
                update_args["dueDateTime"] = (
                    {"dateTime": task_data["due_date"].isoformat(), "timeZone": "UTC"}
    ...
    def create_instruction_task(self, user_id, task_user_email, instructions: str) -> bool:
        ...
        try:
            ...
            try:
                tasks = (
                    self.client.me.outlook.task_folders.by_id(default_folder_id)
                    .tasks.filter(f"subject eq '{self.INSTRUCTION_TASK_TITLE}'")
                    .get()
                    .value
                )
                if tasks:
