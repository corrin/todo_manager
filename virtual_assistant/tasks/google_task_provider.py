...
class GoogleTaskProvider(TaskProvider):
    ...
    def authenticate(self, user_id, task_user_email):
        ...
        if account.needs_reauth:
            logger.info(
                f"Google account for user_id '{user_id}' / task_user_email '{task_user_email}' needs reauthorization"
            )
            ...
            return self.provider_name, redirect(
                url_for(
                    "meetings.reauth_calendar_account",
                    provider="google",
        ...
        try:
            ...
            logger.error(
                f"Google credentials invalid for user_id '{user_id}' / task_user_email '{task_user_email}': {e}"
            )
        ...
        try:
            ...
            for task_list in task_lists:
                ...
                try:
                    ...
                    tasks = (
                        self.client.tasks()
                        .list(tasklist=list_id, showCompleted=False, showHidden=True)
                        .execute()
                        .get("items", [])
                    )
            ...
            logger.error(
                f"Error getting Google tasks for user_id '{user_id}' / task_user_email '{task_user_email}': {e}"
            )
            raise
...
- Handle urgent tasks within 24 hours"""
            ...
            logger.warning(
                f"No AI instruction task found for user_id '{user_id}' / task_user_email '{task_user_email}'"
            )
            return None
        except Exception as e:
            logger.error(
                f"Error getting AI instructions for user_id '{user_id}' / task_user_email '{task_user_email}': {e}"
            )
            raise
    ...
    def update_task(self, user_id, task_id, task_data=None) -> bool:
        ...
        def store_credentials(self, user_id, task_user_email, credentials):
            ...
            raise NotImplementedError(
                "Google Tasks credentials should be stored via the OAuth flow in ExternalAccount."
            )
    ...
    def create_instruction_task(self, user_id, task_user_email, instructions: str) -> bool:
        ...
        try:
            ...
            if instruction_task_id:
                ...
                # using the Google Tasks API
                logger.debug(
                    f"Updated AI instruction task for user_id '{user_id}' / task_user_email '{task_user_email}'"
                )
            else:
                # Create new task
                ...
                # using the Google Tasks API
                logger.debug(
                    f"Created AI instruction task for user_id '{user_id}' / task_user_email '{task_user_email}'"
                )

            return True
        except Exception as e:
            logger.error(
                f"Error managing instruction task for user_id '{user_id}' / task_user_email '{task_user_email}': {e}"
            )
            raise
