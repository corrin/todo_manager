...
class GrokProvider(AIProvider):
    ...
    def authenticate(self, email):
        ...
        if not credentials:
            logger.info(f"No Grok credentials found for {email}")
        ...
        try:
            ...
            logger.debug(f"Grok credentials valid for {email}")
            return None
        except Exception as e:
            logger.error(f"Grok credentials invalid for {email}: {e}")
            return self.provider_name, redirect(url_for('grok_auth.setup_credentials'))
    ...
    def generate_text(self, email, prompt):
        ...
        credentials = self.get_credentials(email)
        if not credentials:
            raise Exception(f"No Grok credentials found for {email}")
