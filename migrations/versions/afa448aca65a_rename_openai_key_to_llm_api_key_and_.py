...
def upgrade():
    op.alter_column(
        "app_user", "openai_key", new_column_name="ai_api_key", existing_type=sa.Text()
    )
...
def downgrade():
    op.alter_column(
        "app_user", "ai_api_key", new_column_name="openai_key", existing_type=sa.Text()
    )
    op.add_column(
        "app_user", sa.Column("ai_provider", sa.String(length=50), nullable=True)
    )
